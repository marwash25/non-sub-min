function [x_primal,x_dual,dual_values,primal_values,gaps,added1,added2,added3] = minimize_submodular_FW_minnormpoint(F,F_true,param_F,maxiter,sfm,gap,optimal_primal,starting_point)
...
if nargin < 8
    ...
    x = greedy_algo_submodular(direction,F,param_F);
else
    x = starting_point;
end
...
while iter < maxiter
    ...
    [xx,Fvalues,order] = greedy_algo_submodular(direction,F,param_F);
    ...
    if sfm
        ...
        primal_values(iter) = F_true(Aopt,param_F); %min(a,0);
        
        % compute traditional value
        added1(iter) = F(find(x<-1e-15),param_F);
        added2(iter) = F(find(x<+1e-15),param_F);
